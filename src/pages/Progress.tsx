          ...
          {progress.badges.length === 0 ? (
            ...
            <div className="flex gap-3 flex-wrap">
              {progress.badges.map((badge) => {
                ...
                return info ? (
                  <motion.div key={badge} initial={{ scale: 0 }} animate={{ scale: 1 }} className="bg-card rounded-2xl p-3 shadow-soft text-center min-w-[80px]">
        ...
        <div>
          ...
          {progress.quizScores.length === 0 ? (
            ...
            <div className="space-y-2">
              {progress.quizScores.slice(0, 5).map((qs, i) => (
                <div key={i} className="bg-card rounded-xl p-3 shadow-soft flex items-center justify-between">
                  ...
                  <div className="font-display text-primary">{qs.score}/{qs.total}</div>
                </div>
              ))}
            </div>
          )}
